...
EXEMPT_CASES = [
    ("state/reviews/deadbeef.json", "panel.py's own artifact, the original reason"),
    ("state/reviews/nested/x.json", "still panel output one level down"),
    ("state/gate-runs.jsonl", "the gate's own run ledger"),
    ("state/bus.jsonl", "a machine-written ledger: a finding names nothing a human can fix"),
    ("state/lessons.jsonl", "same"),
    ("state/claims.jsonl", "same"),
    ("state/compact-log.md", "a generated record, not source"),
    ("tools/review/panel.py", "the reviewer reviewing itself"),
    ("tools/gate/gate.py", "the gate"),
    ("dot-claude/hooks/safety_gate.py", "a hook whose rules are credential patterns"),
]

REVIEWED_CASES = [
    ("state/retired-2026-07-25/bin/generate-visual.py", "REAL tracked source under state/"),
    ("state/retired-2026-07-25/bin/seed-meme-vectordb.py", "its sibling"),
    ("src/state/store.py", "ordinary application code in a very common layout"),
    ("app/state/reducer.ts", "the same, nested"),
    ("tools/map/codemap.py", "a tool the panel does not write"),
]
...
class TestTheRegressionSpecifically:
    def test_the_bare_state_prefix_is_gone(self):
        ...
        """
        src = (Path(__file__).resolve().parents[1] / "tools" / "review" / "panel.py")
        ...
        assert r"^state/(?!" in decl, \
            "the negative lookahead is gone, so state/ is unconditional again"
        # A bare `^state/` means the prefix followed by anything that is NOT the
        # lookahead that carries the correction. `(?!` is the only acceptable suffix.
        assert not re.search(r"\^state/(?!\(\?!)", decl), \
            "a bare ^state/ is back, so tracked source under state/ is unreviewed again"
