...
class HowtoPath(unittest.TestCase):
    def test_a_symlinked_setup_root_does_not_produce_a_climbing_path(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (real / "tools" / "gate").mkdir(parents=True)
            (real / "tools" / "gate" / "gate.py").write_text("", encoding="utf-8")
            ...
            self.assertEqual(got, "python tools/gate/gate.py run --project .")
    ...
    def test_a_genuinely_separate_setup_root_still_gets_a_relative_path(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            setup = Path(td) / "setup"
            (setup / "tools" / "gate").mkdir(parents=True)
            (setup / "tools" / "gate" / "gate.py").write_text("", encoding="utf-8")
            project = Path(td) / "someproject"
            project.mkdir()

            got = hook.howto_command(str(setup), str(project))
            self.assertEqual(got, "python ../setup/tools/gate/gate.py run --project .")
