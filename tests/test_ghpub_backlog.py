...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "tools" / "ghpub"))
...
def test_the_live_backlog_files_render_without_error():
    ...
    sources = sorted((root / "state").glob("github-backlog-*.json"))
...
def test_the_live_backlog_carries_the_keys_the_board_needs():
    root = Path(__file__).resolve().parents[1]
    import json
    data = json.loads((root / "state" / "github-backlog-2026-07-31.json")
