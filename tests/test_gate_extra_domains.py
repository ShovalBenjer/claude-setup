...
GATE_PATH = ROOT / "tools" / "gate" / "gate.py"
...
class GateCase(unittest.TestCase):
    ...
    def install_codemap(self) -> None:
        (self.root / "tools" / "map").mkdir(parents=True, exist_ok=True)
        shutil.copy(CODEMAP_PATH, self.root / "tools" / "map" / "codemap.py")

    def write_map(self) -> None:
        subprocess.run([sys.executable, "tools/map/codemap.py", "write", "--project", self.td],
...
class TestDirMapBuiltin(GateCase):
    def documented(self) -> None:
        self.write("docs/dir-purpose.txt", "\n".join([
            "docs | fixture documentation",
            "tools | fixture tooling",
            "tools/map | the map generator under test",
        ]) + "\n")
    ...
    def test_fails_when_a_directory_has_no_stated_purpose(self):
        ...
        self.assertIn("tools/map", self.evidence_of(report, "codemap"))
        self.assertEqual(1, rc)
    ...
    def test_fails_when_the_map_is_stale(self):
        ...
        self.write("docs/dir-purpose.txt",
                   (self.root / "docs" / "dir-purpose.txt").read_text(encoding="utf-8")
...
class TestPriorArtBuiltin(GateCase):
    ...
    def documented(self) -> None:
        self.write("docs/dir-purpose.txt", "\n".join([
            "docs | fixture documentation",
            "docs/prior-art | build-vs-buy records",
            "engine | the fixture component that owes a record",
            "tools | fixture tooling",
            "tools/map | the map generator under test",
        ]) + "\n")
        self.write("docs/prior-art/out-of-scope.txt",
                   "tools/map | a copy of the tool under test, not fixture code\n")
    ...
    def record(self, **over: object) -> None:
        rec = {
            ...
            "recheck": "python tools/map/codemap.py prior-art",
        }
        rec.update(over)
        self.write("docs/prior-art/engine.json", json.dumps(rec, indent=1))
    ...
    def test_the_gate_shows_what_the_audit_excluded_even_when_it_fails(self):
        ...
        self.assertIn("out of scope tools/map", evidence)
