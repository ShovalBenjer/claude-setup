...
description: >-
  Translate and dub an existing video into another language with voice cloning
  and lip-sync. Use when the source video already exists and the same presenter
  should speak in another language, including video, podcast, and audio-only
  localization. For a new video in another language, use heygen-video instead.
  Not for avatar creation, TTS-only synthesis, or text-only translation.
...
5. **Don't narrate transport choice.** MCP vs CLI is internal. Pick silently at session start; never mention which is in use.
...
1. **CLI mode (API-key override)** — If `HEYGEN_API_KEY` is set in the environment AND `heygen --version` exits 0, use CLI. API-key presence is an explicit signal that the user wants direct API access.
2. **MCP mode** — No `HEYGEN_API_KEY` AND HeyGen MCP tools are visible (`mcp__heygen__*`). OAuth auth, runs against the user's plan credits.
3. **CLI mode (fallback)** — MCP tools not available AND `heygen --version` exits 0. Auth via `heygen auth login`.
