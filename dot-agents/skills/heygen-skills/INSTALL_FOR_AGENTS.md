...
If your agent supports ClawHub, it can short-circuit Steps 1, 3, and 4 below
(clones to the right path, registers with the host, and sets up the install
...
These skills route the actual HeyGen API call through one of two transports.
...
> 1. **CLI (API-key override)** if `HEYGEN_API_KEY` is set AND `heygen --version` exits 0
> 2. **MCP** if no API key set AND `mcp__heygen__*` tools visible
> 3. **CLI (fallback)** if MCP unavailable AND `heygen --version` exits 0
...
### Option A: HeyGen CLI (works with any agent that can shell out)
...
### Option B: HeyGen MCP server (Claude Desktop, Claude Code with MCP)
...
### If workspace identity files are available

If the agent has documented identity files, the skill reads them and designs a
presenter that matches.
...
### If workspace identity files are not available
...
3. **Transport not detected.** Run `heygen --version` (Option A) to confirm the path exists.
...
For the **CLI path** (Option A), defaults live in `~/.heygen/config` — the
CLI will prompt on first use, or you can run `heygen config set` directly.

For the **MCP path** (Option B), there are no per-skill defaults; the agent
...
mode detection ladder occasionally adds new transports.
...
**MCP tools listed but the skill is using the CLI instead.** The
...
transport, unset the env var (`unset HEYGEN_API_KEY`) and re-detect.
