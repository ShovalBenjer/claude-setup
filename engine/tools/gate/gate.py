...
USAGE
  python engine/tools/gate/gate.py init [--project .]        write a starter contract
  python engine/tools/gate/gate.py run [--project .]         run every domain
  python engine/tools/gate/gate.py run --domain e2e          run one domain
  python engine/tools/gate/gate.py status [--project .]      last recorded verdict
  python engine/tools/gate/gate.py selftest
...
LEDGER = os.path.join("knowledge", "state", "gate-runs.jsonl")
...
def starter_contract(project: str) -> dict:
    ...
    c["domains"]["e2e"] = {
        "required": True,
        "cmd": ("python {}/engine/tools/e2e/flow.py audit ${{APP_URL}} --viewport mobile "
    ...
    c["domains"]["review"] = {
        "required": True,
        "artifact": "knowledge/state/reviews/${SHA}.json",
        ...
        "cmd": "python {}/engine/tools/review/panel.py run --project .".format(
            setup_root().replace("\\", "/")),
        "_comment": (
            ...
            "`python engine/tools/review/panel.py run --project . --allow-external` for a "
            "free-model opinion, or payload/dot-claude/bin/external-review-judge.py review "
...
# is not. Measured 2026-07-25: payload/dot-claude/bin/*-launcher.sh held a Key Vault
...
def review_artifact(project: str, contract: dict, spec: dict) -> tuple[str, str]:
    ...
    tmpl = spec.get("artifact", "knowledge/state/reviews/${SHA}.json")
...
def codemap(project: str, spec: dict, sub: str) -> tuple[str, str]:
    """Run engine/tools/map/codemap.py and report what it reported.
    ...
    """
    tool = "engine/tools/map/codemap.py"
