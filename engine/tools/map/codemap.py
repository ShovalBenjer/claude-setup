...
# restructure (payload/engine/knowledge zones). Two earlier attempts at this same
# fix (2026-08-24) knowledge/-prefixed these constants directly and broke every
# fixture in tests/test_codemap.py and tests/test_gate_extra_domains.py, because
# both build a fully synthetic tempdir repo where docs/ is genuinely at the repo
# root, unrestructured on purpose (codemap.py is meant to be generic over
# --project, not specific to this repository's own layout). The fix that survives
# both constraints: real filesystem paths go through fs_path() below, which
# prepends knowledge/ only for a real repo layout; the fixtures were updated in
# the same commit to build their synthetic repos with knowledge/docs/... paths,
# so the tool under test and the tool as deployed agree on the convention.
KNOWLEDGE_DIRS = ("docs/", "work-docs/", "research-papers/")


def fs_path(inventory_path: str) -> str:
    """Inventory-grammar path (docs/..., work-docs/..., research-papers/...) ->
    real on-disk path.

    Mirror of tools/docmap/docmap.py's fs_path(): everything else in this file
    speaks the pre-move grammar (MAP_PATH, REGISTRY_PATH, display strings, error
    messages, GENERATED's exclusion match), but a byte read or write has to hit
    the real location.
    """
...
# The map is a tracked file, real path knowledge/docs/CODEBASE-MAP.md. Counting
# it would make generation clean before its own commit and drifted after it,
# permanently.
GENERATED = (fs_path(MAP_PATH),)
...
def read_registry(project: str) -> dict[str, str]:
    rows: dict[str, str] = {}
    for line in read(os.path.join(project, fs_path(REGISTRY_PATH))).splitlines():
        line = line.strip()
...
def evaluate(project: str) -> dict:
    ...
    wanted = render(dirs)
    on_disk = read(os.path.join(project, fs_path(MAP_PATH)))
...
def problems(state: dict) -> list[str]:
    ...
    if state["drifted"]:
        ...
        out.append("{} is {}. Run `python engine/tools/map/codemap.py write`.{}".format(
...
def read_records(project: str) -> tuple[dict[str, dict], list[str]]:
    found: dict[str, dict] = {}
    broken: list[str] = []
    base = os.path.join(project, fs_path(PRIOR_ART_DIR))
...
def read_scope(project: str) -> list[tuple[str, str]]:
    rows = []
    for line in read(os.path.join(project, fs_path(SCOPE_PATH))).splitlines():
        line = line.strip()
        if not line or line.startswith("#") or "|" not in line:
            continue
...
def cmd_write(args) -> int:
    ...
    dest = os.path.join(args.project, fs_path(MAP_PATH))
