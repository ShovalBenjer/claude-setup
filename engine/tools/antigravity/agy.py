...
AUTH. `GEMINI_API_KEY`, which is the same variable `payload/dot-claude/bin/a2a-gemini-call.py`
...
def blockers(state: dict) -> list[str]:
    ...
    if not state["api_key_set"] and not state["adc_present"]:
        out.append(
            ...
            "payload/dot-claude/bin/a2a-gemini-call.py already uses, free at "
