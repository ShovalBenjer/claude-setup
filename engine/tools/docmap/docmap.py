...
# Inventory-grammar keys: how every document is spelled once tracked_docs() has
# stripped the knowledge/ prefix (see its docstring). Used for classification,
# comparisons, and printed messages -- never joined against `project` directly.
MAP_PATH = "docs/DOCMAP.md"
REGISTRY_PATH = "docs/doc-status.txt"
INDEX_PATH = "docs/INDEX.md"

# Real on-disk locations of the same three files, for actual filesystem reads and
# writes. docs/ lives under knowledge/docs/ on disk since the 2026-08-24 move.
MAP_FS_PATH = "knowledge/" + MAP_PATH
REGISTRY_FS_PATH = "knowledge/" + REGISTRY_PATH
INDEX_FS_PATH = "knowledge/" + INDEX_PATH


# Top-level dirs that moved under knowledge/ on 2026-08-24. The inventory grammar
# (CLASSES, ALWAYS, every selftest fixture) still speaks the pre-move spelling for
# all three; tracked_docs() strips the real prefix on the way in and fs_path() adds
# it back on the way out to an actual filesystem read.
KNOWLEDGE_DIRS = ("docs/", "work-docs/", "research-papers/")


def fs_path(inventory_path: str) -> str:
    """Inventory-grammar path (docs/..., work-docs/..., research-papers/...) -> real
    on-disk path.

    Mirror of the strip done in tracked_docs(): everything else in this file speaks
    the pre-move grammar, but a byte read has to hit the real location.
    """
    if inventory_path.startswith(KNOWLEDGE_DIRS):
        return "knowledge/" + inventory_path
...
def _load_lanes(project: Path):
    """Reuse engine/tools/lib/lanes.py rather than re-deriving the cutover date."""
    p = project / "engine" / "tools" / "lib" / "lanes.py"
    if not p.exists():
        return None
...
def tracked_docs(project: Path) -> list[str]:
    ...
    `docs/` and `work-docs/` live under `knowledge/` on disk (2026-08-24 top-level move), but every
    other function in this file (CLASSES, the selftest fixtures, MAP_PATH-as-inventory-key) speaks
    the pre-move grammar: `docs/adr/...`, `work-docs/...`. Rather than thread `knowledge/` through
    every prefix table and pure-function test in this file, the `knowledge/` prefix is stripped
    right here, at the one place real paths enter the tool. Everything downstream keeps working on
    the grammar it already understood.
    """
    out = subprocess.run(
        ["git", "-C", str(project), "ls-files",
         "knowledge/docs", "knowledge/work-docs", "*.md"],
    ...
    for line in out.split("\n"):
        ...
        if not line:
            continue
        if line.startswith(tuple("knowledge/" + d for d in KNOWLEDGE_DIRS)):
            line = line[len("knowledge/"):]
...
def index_targets(project: Path) -> set[str]:
    ...
    text = read(project / INDEX_FS_PATH)
    out: set[str] = set()
...
def inventory(project: Path) -> list[Doc]:
    ...
    for path in tracked_docs(project):
        text = read(project / fs_path(path))
        cls = classify(path)
...
def evaluate(project: Path) -> dict:
    ...
    return {
        ...
        "on_disk": read(project / MAP_FS_PATH),
    }
...
def problems(state: dict) -> list[str]:
    ...
    if state["rendered"] != state["on_disk"]:
        out.append("{} is not what the documents imply. "
                   "Run `python engine/tools/docmap/docmap.py write`."
                   .format(MAP_PATH))
...
def main() -> int:
    ...
    state = evaluate(project)
    if a.mode == "write":
        (project / MAP_FS_PATH).write_text(state["rendered"], encoding="utf-8")
        n = len(state["docs"])
