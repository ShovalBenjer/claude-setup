...
SELF_REFERENTIAL = re.compile(
    r"(?i)(^|/)(?:engine/tools/(?:review|gate|e2e|refute)/|payload/dot-claude/hooks/)"
    r"|^knowledge/state/(?!.*\.(?:" + SOURCE_EXT + r")$)")
...
SKILL_DEFINITION = re.compile(
    r"(?i)(^|/)SKILL\.md$|(^|/)(?:payload/dot-claude|payload/dot-codex|payload/dot-agents|\.claude)/"
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    dest = args.out or os.path.join(
        project, "knowledge", "state", "reviews", "{}.json".format(sha))
...
def cmd_selftest(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        echoed = {
            "knowledge/state/reviews/abc123.json":
    ...
    probe = [
        {"file": "payload/dot-claude/skills/explain-simply/SKILL.md", "line": 1, "text": "x"},
    ...
    ok = "payload/dot-claude/skills/explain-simply/SKILL.md" not in kept_files \
    ...
    in_skill = [{"file": "payload/dot-claude/skills/x/SKILL.md", "line": 1,
    ...
    key_in_skill = [{"file": "payload/dot-claude/skills/x/SKILL.md", "line": 1,
                     "text": 'AKIA' + "A" * 16}]
    ...
    mixed = [{"file": "src/a.ts", "line": 1, "text": "const x = 1;"},
             {"file": "payload/dot-claude/skills/y/SKILL.md", "line": 1, "text": "# y"}]
