...
The routing half was designed and shipped as dead code. `payload/dot-claude/hooks/prompt-router.sh`
...
def render_context(decision: dict) -> str:
    ...
    if decision["personas"]:
        ...
        if len(decision["personas"]) > 1:
            lines.append("Secondary: " + ", ".join(decision["personas"][1:4]) + ".")
        lines.append(
            "If you delegate, prefer the Agent tool with subagent_type matching that "
            "persona over `general-purpose`. Across 66 transcripts measured 2026-08-05, "
            "0 of 19 company personas had ever been spawned.")
