...
HOOK = os.path.join(os.path.dirname(__file__), "..", "..",
                    "payload", "dot-claude", "hooks", "ship_gate_stop.py")
...
def test_classifier_prose_and_code_paths():
    h = _load()
    assert h.classify_paths(["docs/x.md", "payload/dot-claude/skills/a/SKILL.md", "README.md",
                             "state/gate-runs.jsonl"]) == []
    assert h.classify_paths(["payload/dot-claude/hooks/ship_gate_stop.py"]) == \
        ["payload/dot-claude/hooks/ship_gate_stop.py"]
    assert h.classify_paths(["docs/x.md", "engine/tools/gate/gate.py"]) == \
        ["engine/tools/gate/gate.py"]
...
def test_dirty_docs_over_ungated_code_blocks(tmp_path):
    ...
    g = FakeGate(root, status_lines=[" M docs/notes.md"],
                 diff_since_baseline=["engine/tools/gate/gate.py"])
    docs_only, code = h.is_docs_only(g, "/p")
    assert docs_only is False and code == ["engine/tools/gate/gate.py"]
