...
class LoadGateProjectFirst(unittest.TestCase):
    def test_a_worktree_with_its_own_gate_py_is_preferred_over_the_symlink_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (worktree / "engine" / "tools" / "gate").mkdir(parents=True)
            (worktree / "engine" / "tools" / "gate" / "gate.py").write_text(
                "def setup_root():\n    import os\n"
            ...
            decoy = Path(td) / "decoy"
            (decoy / "engine" / "tools" / "gate").mkdir(parents=True)
            (decoy / "engine" / "tools" / "gate" / "gate.py").write_text(
                "def setup_root():\n    return 'DECOY'\n", encoding="utf-8",
            )

            mod, where = hook.load_gate(str(worktree))
            self.assertEqual(Path(where), worktree / "engine" / "tools" / "gate" / "gate.py")
    ...
    def test_no_project_gate_py_falls_back_to_the_existing_chain(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (setup_root / "engine" / "tools" / "gate").mkdir(parents=True)
            (setup_root / "engine" / "tools" / "gate" / "gate.py").write_text(
                "def setup_root():\n    return 'FALLBACK'\n", encoding="utf-8",
            )
            ...
            self.assertEqual(Path(where), setup_root / "engine" / "tools" / "gate" / "gate.py")
