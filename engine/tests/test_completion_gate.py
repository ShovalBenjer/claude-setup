...
GATE_PATH = ROOT / "payload" / "dot-claude" / "hooks" / "completion_gate.py"
...
class SlopDashTests(unittest.TestCase):
    ...
    def test_the_rule_matches_slop_lint_exactly(self) -> None:
        """Same regex as engine/tools/slop_lint.py:21. Two spellings would drift apart."""
        import re as _re
        slop = ROOT / "engine" / "tools" / "slop_lint.py"
...
class ChannelParityTests(unittest.TestCase):
    ...
    engine/tools/slop_lint.py gates prose FILES; completion_gate.py gates the RESPONSE. Lesson
    ...
    def setUp(self) -> None:
        spec = importlib.util.spec_from_file_location(
            "slop_lint_under_test", ROOT / "engine" / "tools" / "slop_lint.py"
        )
