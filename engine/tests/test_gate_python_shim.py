...
sys.path.insert(0, str(ROOT / "engine" / "tools" / "gate"))
...
class TestTheVirtualenvIsPerPlatform:
    ...
    def test_the_posix_venv_is_gitignored(self):
        ...
        target = "engine/intent-control-plane/" + gate.POSIX_VENV_NAME + "/x"
...
class TestTheOriginalFailure:
    def test_docmap_command_runs_on_a_host_without_python(self):
        ...
        rc, out = gate.run("python engine/tools/docmap/docmap.py check", str(ROOT))
