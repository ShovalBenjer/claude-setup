...
class MapCase(unittest.TestCase):
    ...
    def registry(self, *lines: str) -> None:
        _write(self.root, "knowledge/docs/dir-purpose.txt", "\n".join(lines) + "\n")
...
class TestPurposeResolution(MapCase):
    ...
    def test_purpose_is_one_line_with_table_characters_escaped(self):
        ...
        row = [l for l in (self.root / "knowledge" / "docs" / "CODEBASE-MAP.md").read_text(
    ...
    def test_a_non_ascii_directory_name_is_read_as_itself(self):
        ...
        self.registry("knowledge | Documentation.", "knowledge/docs | Documentation.",
...
class TestCheck(MapCase):
    ...
    def test_check_passes_once_every_dir_resolves(self):
        _write(self.root, "tools/a/x.py", "x = 1\n")
        self.registry("tools | The tool tree.", "tools/a | Does a.", "knowledge | Documentation.", "knowledge/docs | Documentation.")
        self.commit()
        _cli(self.root, "write")
        self.commit("map")
        r = _cli(self.root, "check")
    ...
    def test_a_registry_row_for_a_path_that_is_gone_fails(self):
        ...
        self.registry("tools | The tool tree.", "tools/a | Does a.",
                      "knowledge | Documentation.", "knowledge/docs | Documentation.", "tools/deleted | Did something once.")
        self.commit()
        _cli(self.root, "write")
        self.commit("map")
        r = _cli(self.root, "check")
    ...
    def test_a_registry_row_that_repeats_the_dirs_own_file_fails(self):
        ...
        self.registry("skills | Skill tree.", "knowledge | Documentation.", "knowledge/docs | Documentation.",
    ...
    def test_check_fails_when_the_map_on_disk_is_stale(self):
        ...
        self.registry("tools | The tool tree.", "tools/a | Does a.",
                      "knowledge | Documentation.", "knowledge/docs | Documentation.", "tools/c | Does c.")
        self.commit("added c")
        r = _cli(self.root, "check")
    ...
    def test_check_fails_when_the_map_was_edited_by_hand(self):
        ...
        _cli(self.root, "write")
        self.commit("map")
        m = self.root / "knowledge" / "docs" / "CODEBASE-MAP.md"
    ...
    def test_map_survives_its_own_commit(self):
        ...
        self.commit()
        _cli(self.root, "write")
        before = (self.root / "knowledge" / "docs" / "CODEBASE-MAP.md").read_text(encoding="utf-8")
        self.commit("map")
        self.assertEqual(_cli(self.root, "check").returncode, 0,
                         "committing the map broke the check that generated it")
        _cli(self.root, "write")
        self.assertEqual(before, (self.root / "knowledge" / "docs" / "CODEBASE-MAP.md").read_text(
    ...
    def test_write_is_idempotent(self):
        ...
        self.commit()
        _cli(self.root, "write")
        once = (self.root / "knowledge" / "docs" / "CODEBASE-MAP.md").read_text(encoding="utf-8")
        _cli(self.root, "write")
        self.assertEqual(once, (self.root / "knowledge" / "docs" / "CODEBASE-MAP.md").read_text(
            encoding="utf-8"))
...
class TestPriorArt(MapCase):
    ...
    def record(self, name: str, recheck_after: str, alternatives=None) -> None:
        _write(self.root, "knowledge/docs/prior-art/{}.json".format(name), json.dumps({
    ...
    def test_a_declared_out_of_scope_prefix_removes_the_obligation(self):
        ...
        _write(self.root, "knowledge/docs/prior-art/out-of-scope.txt",
    ...
    def test_an_exclusion_with_no_reason_does_not_count(self):
        self.component("vendored")
        _write(self.root, "knowledge/docs/prior-art/out-of-scope.txt", "tools/vendored\n")
        self.commit()
