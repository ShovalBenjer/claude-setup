...
| dir | files | purpose | from |
| --- | ----: | ------- | ---- |
| `dashboard` | 3 | DASH-1 session dashboard (Tauri v2 + React), slice 3: read-only ledger readers over state/*.jsonl, gate-verdict IPC command and tile, module registry, meme module. src-tauri com... | registry |
...
| `dashboard/core/src` | 3 | Rust source for dashboard-core: lib.rs (LedgerReadReport, IpcError) plus the ledger/, meme, modules submodules | registry |
...
| `docs/analysis` | 65 | Point-in-time analysis writeups (per docs/INDEX.md heading) feeding the TODO list: stress tests, gap audits, cost/free-tier notes | registry |
...
| dir | files | purpose | from |
| --- | ----: | ------- | ---- |
| `tests` | 49 | Single pytest file exercising the prove-implementation proof/loop-audit scripts (imported from dot-claude/skills) | registry |
