...
|---|--------|---------------------|--------|----------|
| 1 | AUTO-01 | Any fresh session reaches full working context in <60s from disk only (SESSION-BOOT.md path) | STAGED | DOWNGRADED from DONE 2026-07-24 (calibration loss L011): hook never fired in-harness — bare `bash` on Windows PATH = WSL bash, and exec-form backslash args arrived mangled. Fixed: explicit Git Bash interpreter + POSIX /c/ arg; exec-green from a native Windows parent (exit 0, 4621 bytes). DONE requires a harness-written `state/hook-fires.log` SessionStart line, not a pipe test |
| 2 | AUTO-02 | Compaction/clear loses zero decisions: PreCompact hook writes durable snapshot; disk is the only memory (ADR-0010) | STAGED | DOWNGRADED from DONE 2026-07-24 (L011): same broken interpreter/path form, so no in-harness snapshot ever ran. Steering correctly lives in SessionStart source=compact (PreCompact has no additionalContext — verified). DONE requires a fire-log PreCompact line from a real /compact |
