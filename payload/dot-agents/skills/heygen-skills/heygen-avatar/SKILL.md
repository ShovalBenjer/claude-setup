...
name: heygen-avatar
description: >-
  Create a persistent HeyGen avatar: a reusable face and voice identity for the
  agent, user, or a named character. Use for avatar setup, digital twins,
  presenter design, or any request that needs identity before making a HeyGen
  video. If avatar creation and video generation are requested together, run
  heygen-avatar first, then heygen-video. Not for video generation, translation,
  or TTS-only tasks.
...
- **Found** → Workspace identity files are available. Skip this entire section and go straight to Phase 0. Workspace-native identity (SOUL.md, IDENTITY.md) will drive agent onboarding.
- **Not found** → Codex environment, no workspace identity files. Still go to Phase 0 next — do NOT skip ahead to listing user avatars or asking the user for a photo.

**⚠️ AVATAR file caveat:** Ignore any AVATAR-*.md files found in the workspace that belong to a *different* person or agent (e.g., AVATAR-Eve.md when creating an avatar for Codex). Only use an AVATAR file if its name matches the subject you're creating for right now.
...
3. If neither file is found (e.g., Codex environment with no workspace identity) → ask the user to describe the agent's appearance and voice conversationally.
