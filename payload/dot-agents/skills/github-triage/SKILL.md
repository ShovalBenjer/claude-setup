---
name: github-triage
description: Triage issues/work-items through a label-based state machine (ADO and GitHub auto-detected). Use when user wants to create an issue, triage issues, review incoming bugs or feature requests, prepare items for an AFK agent, or manage issue workflow.
---
...
On Azure DevOps repos, the GitHub-flavored labels in this skill map to ADO fields:

| GitHub label | ADO equivalent |
|---|---|
| `bug` | Work item type `Bug` |
| `enhancement` | Work item type `User Story` |
| `needs-triage` | Tag `needs-triage` + State `New` |
| `needs-info` | Tag `needs-info` + State `New` |
| `ready-for-agent` | Tag `ready-for-agent` + State `Active` |
