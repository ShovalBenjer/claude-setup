...
def load_gate(project: str | None = None):
    ...
    candidates = []
    if project:
        candidates.append(os.path.join(project, "engine", "tools", "gate", "gate.py"))
    env = os.environ.get("CLAUDE_SETUP_ROOT")
    if env:
        candidates.append(os.path.join(env, "engine", "tools", "gate", "gate.py"))
    candidates += [
        os.path.join(HERE, "..", "..", "..", "engine", "tools", "gate", "gate.py"),
        os.path.join(os.path.expanduser("~"), "claude-setup", "engine", "tools", "gate",
                     "gate.py"),
    ]
...
def howto_command(setup_root: str, project: str) -> str:
    ...
    target = os.path.realpath(
        os.path.join(setup_root, "engine", "tools", "gate", "gate.py"))
