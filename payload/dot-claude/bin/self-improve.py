...
#
# This file is payload (payload/dot-claude/bin/self-improve.py) deployed to
# ~/.claude/bin/, but it is also runnable straight out of a claude-setup checkout at
# payload/dot-claude/bin/self-improve.py, three levels below the repo root, matching
# engine/tools/bus/backfill_session_telemetry.py and engine/tools/intent/capture_turn.py.
# It used to insert $HOME/projects/intent-control-plane/src, a path that has never
# existed on this machine (there is no ~/projects at all), so the import below always
# raised ModuleNotFoundError with no indication of what was tried. Resolve
# repo-relatively instead, and fail loudly and specifically if that resolution is
# wrong, rather than falling through to a confusing import error two lines down.
_ICP_SRC = Path(__file__).resolve().parents[3] / "engine" / "intent-control-plane" / "src"
if not _ICP_SRC.is_dir():
    raise SystemExit(
        f"self-improve.py: intent-control-plane/src not found at {_ICP_SRC} "
        f"(resolved as ../../../engine/intent-control-plane/src from "
