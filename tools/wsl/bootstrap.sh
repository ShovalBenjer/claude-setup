...
# and Ubuntu ships no bare `python` at all, so on WSL the repo's own documented commands
# die with "command not found". Two attempts, in order.
#
...
# run, which contradicts the resumability this file's header promises. Best-effort here,
# then the symlink catches every case where it did not land. Putting it in APT_WANT made
# the fallback unreachable in precisely the situations it exists for.
...
# Same idiom as fdfind above, and unprivileged, so `python tools/...` resolves either way.
