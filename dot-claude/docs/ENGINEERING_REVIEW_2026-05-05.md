# Engineering Review Sweep - 2026-05-05

Automation id: `c02-engineering-review-sweep`
Run timestamp UTC: `20260505T172847Z`
Workspace root: `/home/shovalbe`

## EXECUTIVE_SUMMARY

- Local repos scanned: 17 git roots under `/home/shovalbe/projects`.
- Specs scanned: 41 markdown specs across home/project spec directories.
- Static/lint commands attempted where configured: 2.
- Azure DevOps remote coverage: auth-needs-refresh.
...
### BLOCKED
- Azure DevOps CLI returned auth-needs-refresh even though Azure account auth is present.
- Evidence: `az repos list` and `az repos pr list --status active` both returned: `Before you can run Azure DevOps commands...`.

## AZURE_DEVOPS_REMOTE_COVERAGE

- Organization/project configured: `https://dev.azure.com/Corp-domain` / `Corp-AI`.
- `az account show` succeeded for an AAD identity, but Azure DevOps CLI auth failed; recorded status: `auth-needs-refresh`.
- Active PRs, stale PRs, remote branches older than 30 days, and uncloned ADO repos could not be enumerated in this run.
- No ADO comments or work items were created because no remote PR/work item could be confidently identified.
...
- `projects/_inherited/automation-fabric`: branch `main`, origin `https://dev.azure.com/Corp-domain/Corp-AI/_git/automation-fabric`, status `clean`, remote heads read `0` rc `128`.
...
- `projects/campaign-analysis`: branch `fix/healthcheck-livez-path`, origin `https://dev.azure.com/Corp-domain/Corp-AI/_git/campaign-analysis`, status `dirty`, remote heads read `0` rc `128`.
- `projects/campaign-analysis/Onesignal`: branch `fix/healthcheck-livez-path`, origin `https://dev.azure.com/Corp-domain/Corp-AI/_git/campaign-analysis`, status `dirty`, remote heads read `0` rc `128`.
...
- `projects/cs-agent/axia-seekapa-cs-agents-devops`: branch `feat/v109.5-yasha-style-canonical-refinement`, origin `https://dev.azure.com/Corp-domain/Corp-AI/_git/axia-seekapa-cs-agents`, status `dirty`, remote heads read `0` rc `128`.
...
- `projects/seekapa-training-platform`: branch `fix/ci-manylinux-wheels`, origin `NO_ORIGIN`, status `dirty`, remote heads read `0` rc `n/a`.
...
- ruff check failed or unavailable (rc=2).
- Scan `broad-except` hits: 9 sample(s).
...
- Verification: `uv run ruff check` -> rc `2`; sample: error: Request failed after 3 retries in 4.5s |   Caused by: Failed to fetch: `https://pypi.org/simple/python-multipart/` |   Caused by: error sending request for url (https://pypi.org/simple/python-multipart/)
...
- Scan `high-z-index` hits: 0 sample(s).
- Scan `important` hits: 0 sample(s).
- Scan `sleep-poll` hits: 3 sample(s).
  - `projects/social-intelligence-unit/.kilocode/orchestrator/kilo_orchestrate.py:147: time.sleep(2)`
  - `projects/social-intelligence-unit/.kilocode/skills/regression-oracle/capture.py:87: time.sleep(1)`
  - `projects/social-intelligence-unit/.kilocode/skills/regression-oracle/replay.py:114: time.sleep(1)`
...
- `index on feat/align-test-connector-to-production: 1510f768 feat(platform): Ink Skill Cards TUI + Stop banner — live` by `Shoval Be` appears 3 times; SHAs 6ac6ffcf, 6ac6ffcf, 6ac6ffcf; repos projects, projects/cs-agent, projects/qc
- `untracked files on feat/align-test-connector-to-production: 1510f768 feat(platform): Ink Skill Cards TUI + Stop banner — live` by `Shoval Be` appears 3 times; SHAs ffc0ff85, ffc0ff85, ffc0ff85; repos projects, projects/cs-agent, projects/qc
- `Merge pull request 184 from feat/v108-yasha-intake into master` by `Shoval Benjer` appears 4 times; SHAs d4efc6d9, d4efc6d9, d4efc6d9, d4efc6d9; repos projects, projects/cs-agent, projects/cs-agent/axia-seekapa-cs-agents-devops, projects/qc
- `feat(seekapa): v108 yasha intake flow — live (agent v113)` by `Shoval Benjer` appears 4 times; SHAs 6c82fb55, 6c82fb55, 6c82fb55, 6c82fb55; repos projects, projects/cs-agent, projects/cs-agent/axia-seekapa-cs-agents-devops, projects/qc
- `Merged PR 178: fix(handler): dedupe banned-verb subs + escalation-aware identity strip + contact extractor` by `Shoval Benjer` appears 4 times; SHAs 07b3d683, 07b3d683, 07b3d683, 07b3d683; repos projects, projects/cs-agent, projects/cs-agent/axia-seekapa-cs-agents-devops, projects/qc
...
- `projects/.archive/Oded/archive/taqyeem-broker-website/azure-pipelines.yml`: CI auth reproducer risk strings
...
- `rtk find /home/shovalbe/projects ... -name .git`: found 17 git roots; RTK find wrapper ignored GNU flags, but returned repo roots.
- `rtk az account show --query ... -o json`: rc 0; AAD identity present.
- `az repos list -o json`: rc 1; DevOps auth-needs-refresh error.
- `az repos pr list --status active -o json`: rc 1; DevOps auth-needs-refresh error.
- `projects/campaign-analysis`: `uv run ruff check` rc `2`.
