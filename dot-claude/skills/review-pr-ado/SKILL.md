---
name: review-pr-ado
description: PR precheck review — composites codex-call code review + testing-pyramid gap plan + heidegger-reflect into a single structured thread posted to Azure DevOps via the azure-devops Python SDK or `az devops invoke`. Triggers on "/review-pr-ado", "before merge", "PR precheck", "review this PR", "review PR #<n>".
skip-if: >
...
> **INACTIVE — former-employer context.** This skill was written against a specific
> former employer's Azure DevOps org and project, and an Azure AI Foundry account
> and resource group, that are confirmed dead (see
> `dot-claude/rules/project-template/foundry-deployment-per-project.md`,
> STATUS 2026-08-18: SUPERSEDED). It is kept only as a reference implementation for
> the ADO-thread-posting mechanics (`az devops invoke` / `pullRequestThreads` REST
> shape, dedup-on-rerun, key-vs-AAD-token auth pattern) should a live ADO target
> reappear. Do not run this against any current project without first replacing
> every org/project/repo/account name below with a live, verified target. The
> generic, current, multi-layer review scorecard lives at
> `dot-claude/skills/review/SKILL.md`.

# /review-pr-ado — PR precheck (Azure DevOps)
...
|---|---|---|---|
| **Interactive** | Developer runs `/review-pr-ado` in Claude Code | This SKILL.md (steps 1–7 below) | `work-item.sh` or azure-devops SDK |
...
**Check / grant (example org/project/repo — replace with a live target):**
```
ADO Portal → Project Settings → Repos → <repo>
           → Security → "<project> Build Service (<org>)"
...
- Before merging any branch to master/main, in a repo whose remote is Azure DevOps
- User types `/review-pr-ado`, "review PR #N", "precheck before merge" in an ADO repo
- Automated overnight precheck (Gastown job)

## When NOT to use

- Diff is empty
- No ADO remote (GitHub-only repo — use `gh pr review` directly, or the generic
...
```bash
BASE=$(jq -r '.review.base_branch // "main"' .claude/projects.json 2>/dev/null || echo "main")
...
_Generated by /review-pr-ado skill — <ISO8601 timestamp> — diff base: origin/<BASE>_
...
## CI path — the fleet flow (comment handling; reference implementation, not live)

The original deployment ran a `CodexReview` stage on `Build.Reason == PullRequest` in
every active project's pipeline, reviewing the diff and posting findings as PR comment
threads. This is the pre-PR flow: the reviewer talks to the PR in comments, not a
wall-of-text. Kept here as a worked example of the mechanics; the specific account
and resource-group names this originally ran against are dead and must not be reused as-is.
...
The pipeline service principal typically has Contributor (control-plane) but not a
data-plane role on the AI resource, so `DefaultAzureCredential` → `responses.create`
can return **403**. The workaround: fetch the AIServices key in-stage (Contributor can
`listKeys`) and prefer it in the script:

```bash
export AZURE_AI_FOUNDRY_KEY="$(az cognitiveservices account keys list -n <account> -g <resource-group> --query key1 -o tsv)"
...
### Model — codex-family deployment

Use whatever codex-family model deployment the live account exposes for code review,
not the general/chat-tier model — the two are usually versioned differently in the same
catalog and only the codex one is tuned for review.
